...
const routes = [
  '/child','/child/aprender','/child/axion','/child/games','/child/store','/child/stickers','/child/notifications','/child/help','/child/profile','/child/achievements','/child/settings','/child/games/quiz','/child/games/memory','/child/games/finance-sim','/child/games/tictactoe','/child/games/wordsearch','/child/games/tug-of-war',
];
  ...
  for (const route of routes) {
    ...
    try {
      ...
      const diagnostics = await page.evaluate(() => {
        ...
        const sidebarStyles = fixedLeftAside ? window.getComputedStyle(fixedLeftAside) : null;
        const sidebarBg = sidebarStyles?.backgroundImage || '';
        const sidebarTrailLike = sidebarBg.includes('linear-gradient') && (sidebarBg.includes('6, 18, 39') || sidebarBg.includes('4, 13, 30'));
        return {
      ...
      item = { ...item, ok: true, screenshotPath, consoleErrorCount: consoleErrors.length, responseErrorCount: responseErrors.length, consoleErrors: consoleErrors.slice(0,8), responseErrors: responseErrors.slice(0,12), ...diagnostics };
    } catch (error) {
...
(async () => {
  ...
  const mobileContext = await browser.newContext({ viewport: { width: 390, height: 844 }, storageState: storagePath });
