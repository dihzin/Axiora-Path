...
const NODE_GAP = 70;
const NODE_GAP_MOBILE = 92;
const TOP_PAD = 40;
const BOTTOM_LABEL_PAD = 52;
...
function getNodeX(index: number) {
  const pattern = [0, 240, -240, 170, -170, 120, -120, 80, -80];
...
function clampNodeOffset(offset: number, viewportWidth: number) {
  const maxOffset = Math.max(84, viewportWidth / 2 - 96);
...
function MapNodeItem({ node, isActive, displayIndex, onClick }: MapNodeItemProps) {
  ...
  return (
    <div className="group relative">
      <div className="pointer-events-none absolute -top-2 left-1/2 z-30 w-max max-w-[240px] -translate-x-1/2 -translate-y-full rounded-xl border border-white/16 bg-black/30 px-3 py-2 text-center text-[11px] text-white/90 opacity-0 shadow-[0_6px_18px_rgba(0,0,0,0.2)] backdrop-blur-md transition-opacity duration-150 group-hover:opacity-100">
      ...
      <button
        ...
        className={cn(
          ...
          isLocked ? "border border-white/15 bg-white/10 text-white/70 backdrop-blur" : "",
      ...
      <p className="pointer-events-none absolute bottom-[calc(100%+0.5rem)] left-1/2 w-max max-w-[160px] -translate-x-1/2 text-center text-[12px] font-medium leading-tight tracking-tight text-white/85">
        {node.title}
      </p>
    </div>
  );
  ...
  return (
    ...
    >
      <div className="relative flex -translate-x-1/2 -translate-y-1/2 flex-col items-center">
        <MapNodeItem node={node} isActive={isActive} displayIndex={nodeIndex + 1} onClick={() => onNodeClick?.(node)} />
      </div>
    </div>
  );
}
...
export default function ProgressionMap({
  ...
  const points = nodes.map((_, gi) => ({
    x: trackCenter + clampNodeOffset(Math.round(getNodeX(gi) * offsetScale), viewportWidth),
  ...
  const maxOffsetX = useMemo(
    () =>
      nodes.reduce(
        (acc, _, gi) =>
          Math.max(acc, Math.abs(clampNodeOffset(Math.round(getNodeX(gi) * offsetScale), viewportWidth))),
        0,
      ),
  ...
  return (
    <div className={cn("relative w-full overflow-hidden py-10", className)} style={{ maxHeight: "none" }}>
      <div className="relative w-full overflow-visible" style={{ height: `${mapHeight}px`, maxHeight: "none" }}>
        ...
        >
          ...
          {progressPath ? (
            <path
              ...
              fill="none"
              strokeLinecap="round"
              style={{ filter: "drop-shadow(0 0 18px rgba(56,189,248,0.7))" }}
            />
          ) : null}
        </svg>
        <div
        ...
        {nodes.map((node, index) => {
          ...
          return renderNode(node, point, index, trackCenter, selectedNodeId ?? activeNodeId, onNodeClick);
        })}
      </div>
      {showDebugOverlay ? (
