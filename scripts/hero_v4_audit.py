...
def main() -> int:
    ...
    # No backend mutation in changed visual components.
    ...
    # Tabs removed from the top (UI marker: no AREA_LABELS.map rendering block).
    tabs_removed = "AREA_LABELS.map" not in trail and "setSelectedArea(area)" not in trail
    if not tabs_removed:
        failures.append(("Tabs Removed", "area tabs render markers still present"))

    # Subject selector still functional and layered above content.
    selector_functional = all(
        marker in selector
        for marker in [
            'aria-controls="subject-selector-listbox"',
            "id=\"subject-selector-listbox\"",
            "onSelectSubject(subject.id)",
            "z-[110]",
        ]
    )
    if not selector_functional:
        failures.append(("Subject Selector Functional", "selector listbox/select or layering markers missing"))

    # XP integrity: Hero only formats incoming props, no local XP accumulation/recalculation loops.
    xp_integrity = all(
        token not in hero
        for token in [
            "fetch(",
    ...
    print("✔ Tabs Removed")
    print("✔ Subject Selector Functional")
